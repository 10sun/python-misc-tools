...
from pandas.tseries.offsets import CustomBusinessDay, Day, BusinessDay, BDay
...
def non_unique_bin_edges_error(func):
    """give user a more informative error in case it is not possible to properly calculate quantiles on the input dataframe
...
def quantize_data(data, quantile:int=5, bins=None, by_group=False, no_raise=False, zero_aware=False):
    """_summary_

    Args:
        data (_type_): _description_
        quantile (int, optional): _description_. Defaults to 5.
        bins (_type_, optional): int or sequence [float]. Defaults to None.
        by_group (bool, optional): If True, compute quantile buckets separately for each group. Defaults to False.
        no_raise (bool, optional): _description_. Defaults to False.
        zero_aware (bool, optional): _description_. Defaults to False.
    """
    if not ((quantile is not None and bins is None) or
        (quantile is None and bins is not None)):
        raise ValueError('Either quantiles or bins should be provided')

    if zero_aware and not (isinstance(quantile, int)
    ...
    data_quantile = data.groupby(grouper)['factor'] \
        .apply(quantile_calc, quantile, bins, zero_aware, no_raise)
...
def diff_custom_calendar_timedeltas(start, end, freq, date_freq='D'):
    ...
    if weekmask is not None and holidays is not None:
        ...
        actual_periods = np.busday_count(np.array(start).astype('datetime64[D]'),
...
def rate_of_return(period_ret, base_period):
    ...
    ### TODO: check why a conversion factor is needed
...
def factor_alpha_beta(factor_data,
    ...
    # mean of fwd return of universe at each date
...
def positions(weights, period, freq=None):
    ...
    returns_idx = add_custom_calendar_timedelta(trades_idx, period, freq)
    ...
    for curr_time in weights_idx:
        ...
        if curr_time in weights.index:
            ...
            expire_ts = add_custom_calendar_timedelta(curr_time,
                                                            period, freq)
...
#### TODO: this could be useful for showing the percentile diff
...
#### TODO: check how to use this func
...
def get_turnover_table(autocorrelation_data, quantile_turnover):
...
"""
mean_quant_ret, std_quantile = sig_analysis.mean_return_by_quantile(
    region_tsmom_data,
    by_group=False,
    demeaned=True,
    group_adjust=False,
...
mean_quant_rateret = mean_quant_ret.apply(
    sig_analysis.rate_of_return, axis=0, base_period=mean_quant_ret.columns[0]
)
"""
def plot_quantile_returns_bar(mean_ret_by_q,
    ...
    if by_group:
        ...
        if fig is None:
            ...
            print(mean_ret_by_q.index)
            print(mean_ret_by_q.values)
            fig.fig.add_trace(
                go.Bar(
                    x=mean_ret_by_q.index, y=mean_ret_by_q.values
                ),
                row=fig.next_row() if fig.curr_row > 1 else fig.curr_row,
                col=fig.curr_col,
            )
            fig.fig.show()
        #return ax
...
def plot_cumulative_returns(factor_returns,
    ...
    factor_returns = cumulative_returns(factor_returns)
...
def plot_cumulative_returns_by_quantile(quantile_returns,
    ...
    cum_ret = ret_wide.apply(cumulative_returns)
...
class GridFigure(object):
    ...
    def __init__(self, rows, cols):
        self.rows = rows
        self.cols = cols
        self.fig = make_subplots(rows, cols, shared_xaxes=True)
...
def create_summary_tear_sheet(
    ...
    fr_cols = 0# len(periods)
    vertical_sections = 3 + fr_cols * 3
    gf = GridFigure(rows=vertical_sections, cols=1)
    
    """
    display(get_quantile_statistics_table(factor_data))

    display(get_returns_table(
    ...
    rows = vertical_sections#6 + len(perf_summary['yearly returns'].columns)
    row_heights=[0.35, 0.35, 0.5]#, 0.2, 0.2, 0.2]

    for i in range(fr_cols):
        specs += [[{"colspan": 2}, None],[{"colspan": 2}, None],[{}, {}]]
        row_heights += [0.3, 0.3, 0.6]
    ...
    plotly_charts.add_table(fig, round(get_quantile_statistics_table(factor_data), 3), row=1)

    plotly_charts.add_table(fig, round(get_returns_table(
        alpha_beta, mean_quant_rateret, mean_ret_spread_quant
    ),3), row=2)

    display(mean_quant_rateret)
    plotly_charts.add_bars(fig, mean_quant_rateret*DECIMAL_TO_BPS, orientation='v', row=3, yaxis_title='Mean Return (bps)')
    
    display(mean_quant_rateret_bydate[1])
    ...
    for i in range(fr_cols):    
        plotly_charts.add_curve(fig, mean_quant_rateret_bydate[(1)], row=3*(i+1)+1)
        plotly_charts.add_curve(fig, mean_quant_rateret_bydate, row=3*(i+1)+2)
        plotly_charts.add_curve(fig, mean_quant_rateret_bydate, row=3*(i+1)+3, col=1)
        plotly_charts.add_curve(fig, mean_quant_rateret_bydate, row=3*(i+1)+3, col=1)
    ...
    display(get_information_table(ic))
    ...
    autocorrelation = pd.concat(
        [
            factor_rank_autocorrelation(factor_data, period)
            for period in periods
        ],
        axis=1,
    )

    display(get_turnover_table(autocorrelation, quantile_to))
...
def create_returns_tear_sheet(
    ...
    alpha_beta = factor_alpha_beta(
        factor_data, factor_rets, long_short, group_neutral
    )
    ...
    fr_cols = len(factor_rets.columns)
    ...
    for col in factor_rets.columns:
        ...
        plot_cumulative_returns(
            factor_rets[col], period=col, title=title, ax=gf.next_row()
        )
...
def create_information_tear_sheet(
    ...
    ic = factor_information_coefficient(factor_data, group_neutral)
    ...
    if not by_group:
        ...
        mean_monthly_ic = mean_information_coefficient(
            factor_data,
            group_adjust=group_neutral,
            by_group=False,
            by_time="M",
    ...
    if by_group:
        mean_group_ic = mean_information_coefficient(
            factor_data, group_adjust=group_neutral, by_group=True
        )

        plot_ic_by_group(mean_group_ic, ax=gf.next_row())
...
def create_turnover_tear_sheet(factor_data, turnover_periods=None):
    ...
    if turnover_periods is None:
        input_periods = get_forward_returns_columns(
            factor_data.columns, require_exact_day_multiple=True,
        ).get_values()
        turnover_periods = timedelta_strings_to_integers(input_periods)
    else:
        turnover_periods = timedelta_strings_to_integers(
            turnover_periods,
        )
    ...
    get_turnover_table(autocorrelation, quantile_turnover)
...
def create_full_tear_sheet(factor_data,
    ...
    get_quantile_statistics_table(factor_data)
...
def create_event_returns_tear_sheet(factor_data,
    ...
    avg_cumulative_returns = average_cumulative_return_by_quantile(
        factor_data,
        returns,
    ...
    if by_group:
        ...
        avg_cumret_by_group = average_cumulative_return_by_quantile(
            factor_data,
            returns,
            periods_before=before,
            periods_after=after,
